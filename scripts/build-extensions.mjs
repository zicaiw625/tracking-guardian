  ...
  targets.forEach(({ path: filePath, label, required }) => {
    ...
    try {
      ...
      if (result.updated) {
        ...
        console.log(
          `✅ ${label} updated (${result.previousValue ?? "placeholder"} -> ${result.nextValue ?? PLACEHOLDER})`
        );
        updatedCount++;
      } else {
        const reason =
          result.reason === "placeholder_missing"
...
function resolveBackendUrl() {
  const candidates = [process.env.RENDER_EXTERNAL_URL, process.env.PUBLIC_APP_URL].filter(Boolean);
...
function injectBackendUrl() {
  ...
  const updatedCount = processConfigFiles(
    ...
    (content) => {
      ...
      return result.updated ? { ...result, nextValue: backendUrl } : result;
    }
  );
  if (updatedCount === 0) {
