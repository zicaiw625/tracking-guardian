...
describe("API Version Consistency", () => {
  function getAppTomlVersion(): string | null {
    const content = readFileSync(resolve(__dirname, "../shopify.app.toml"), "utf-8");
    const match = content.match(/api_version\s*=\s*"([^"]+)"/);
    return match ? match[1] : null;
  }
  function getExtensionTomlVersion(): string | null {
    const content = readFileSync(resolve(__dirname, "../extensions/tracking-pixel/shopify.extension.toml"), "utf-8");
    const match = content.match(/api_version\s*=\s*"([^"]+)"/);
    return match ? match[1] : null;
  }
  function getServerVersion(): string | null {
    const content = readFileSync(resolve(__dirname, "../app/services/shopify/app-config.server.ts"), "utf-8");
