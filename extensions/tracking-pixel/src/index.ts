...
register(({ analytics, settings, init, customerPrivacy }: any) => {
    ...
    async function sendToBackend(eventName: string, data: Record<string, unknown>): Promise<void> {
        if (!hasFullConsent()) {
            log(`Skipping ${eventName} - insufficient consent. ` +
                `analytics=${analyticsAllowed}, marketing=${marketingAllowed}, saleOfData=${saleOfDataAllowed}`);
            return;
        }
        log(`${eventName}: Sending to backend. ` +
    ...
    analytics.subscribe("product_added_to_cart", (event) => {
        ...
        if (!cartLine?.merchandise?.product)
            return;
