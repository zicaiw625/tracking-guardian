...
### (A) 免费：Audit 风险报告（必须强）
- **自动扫描**：通过 Shopify API 读取已安装的 ScriptTags 和**本应用的 Web Pixel 配置**。
  - **ScriptTags**：可扫描全店（历史遗留）
  - **Web Pixel**：仅检测/管理**本应用的 Web Pixel record**（安装状态/配置正确性），无法扫描其他 App 的 Web Pixels。
  - **不包含** checkout.liquid / Additional Scripts（需手动粘贴）。
...
- **迁移验收报告导出（多店铺）**：生成多店铺迁移报告（CSV）*Coming soon (v1.1)*
...
| 套餐 | 定价 | 适用场景 | 核心能力 |
| --- | --- | --- | --- |
| **Free** | $0 | 扫描、评估阶段 | Audit、迁移清单与建议、风险分级与替代路径、截止日期提醒 |
| **Starter** | **$29/月** | 像素迁移阶段 | 像素迁移（1 平台）、Survey 或 Helpdesk 二选一、基础验收（不含报告导出）、标准事件映射 + 参数完整率、可下载 payload 证据、Test/Live 环境切换 |
| **Growth** | **$79/月** | 项目交付阶段 | 导出报告（CSV）+ 监控告警、像素迁移 + 模块发布、事件参数完整率、订单金额/币种一致性验证、隐私合规检查、每月 10,000 笔订单追踪 |
| **Agency** | **$199/月** | 多店代理/协作 | 无限像素、全部模块、验收报告导出（CSV）、每月 100,000 笔订单、SLA、专属客户成功经理 |
