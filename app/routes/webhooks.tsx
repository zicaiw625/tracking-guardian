...
async function processOrderConversion(
  ...
  // This same eventId may have been generated by the pixel endpoint
  // Using the same algorithm ensures pixel + webhook events dedupe correctly
  ...
  for (const pixelConfig of shopRecord.pixelConfigs) {
    ...
    const existingLog = await prisma.conversionLog.findUnique({
      where: {
        shopId_orderId_platform_eventType: {
          shopId: shopRecord.id,
          orderId: orderId,  // Use normalized orderId
    ...
    const conversionLog = await prisma.conversionLog.upsert({
      ...
      update: {
        ...
        eventId: eventId,  // Ensure eventId is set for CAPI dedup
      },
      create: {
        shopId: shopRecord.id,
        orderId: orderId,  // Use normalized orderId
        eventId: eventId,  // Store eventId for CAPI dedup
    ...
    try {
      ...
      const conversionData: ConversionData = {
        orderId: orderId,  // Use normalized orderId for consistency
        orderNumber: order.order_number ? String(order.order_number) : null,
      ...
      // Pass the unified eventId for platform-level deduplication
      // This ensures pixel events and webhook events dedupe correctly on the platform side
      switch (pixelConfig.platform) {
        case "google":
          result = await sendConversionToGoogle(
            decryptedCredentials as GoogleCredentials | null,
            conversionData,
            eventId  // Pass eventId for GA4 transaction_id dedup
          );
          break;
        case "meta":
          result = await sendConversionToMeta(
            ...
            conversionData,
            eventId  // Pass eventId for Meta event_id dedup
          );
          break;
        case "tiktok":
          result = await sendConversionToTikTok(
            ...
            conversionData,
            eventId  // Pass eventId for TikTok event_id dedup
          );
          break;
        default:
