    ...
    if (shop) {
      safeFireAndForget(
        trackEvent({
          ...
          metadata: confirmation.success ? { plan: confirmation.plan } : { error: confirmation.error },
        })
      );
    }
...
export const action = async ({ request }: ActionFunctionArgs) => {
  try {
    ...
    return json({ success: false, error: "Authentication failed, please refresh and try again" }, { status: 401 });
  }
};
...
export default function BillingPage() {
  ...
  return (
    <Page title={t("billing.pageTitle")}>
      <BlockStack gap="500">
        {showSuccessBanner && (
          <Banner title={t("billing.successTitle")} tone="success" onDismiss={() => {}}>
            <p>{t("billing.successMessage")}</p>
          </Banner>
        )}

        {showErrorBanner && errorMessage && (
          <Banner title={t("billing.failTitle")} tone="critical" onDismiss={() => {}}>
            <p>{errorMessage}</p>
          </Banner>
        )}
        ...
        {subscription.isTrialing && (
          <Banner title={t("billing.trialBannerTitle")} tone="info">
            <p>
              {t("billing.trialBannerMessage", {
                planName: t(currentPlan.name),
                days:
                  (subscription as typeof subscription & { trialDaysRemaining?: number; trialDays?: number })
                    .trialDaysRemaining ??
                  (subscription as typeof subscription & { trialDaysRemaining?: number; trialDays?: number })
                    .trialDays ??
                  0,
        ...
        {usage.exceeded && (
          <Banner title={t("billing.limitBannerTitle")} tone="critical">
            <p>{t("billing.limitBannerMessage", { limit: usage.limit })}</p>
          </Banner>
        )}
            ...
            <Card>
              <BlockStack gap="400">
                <InlineStack align="space-between">
                  ...
                  <Badge tone={subscription.hasActiveSubscription ? "success" : "info"}>{t(currentPlan.name)}</Badge>
                </InlineStack>
                <Divider />
                <BlockStack gap="200">
                  <InlineStack align="space-between">
                ...
                <BlockStack gap="200">
                  <Text as="span" variant="headingSm">
                    {t("billing.featuresTitle")}
                  </Text>
                  <List>
                    {currentPlan.features.map((feature, index) => (
                      <List.Item key={index}>{renderFeature(feature)}</List.Item>
                    ))}
                  </List>
                </BlockStack>
                  ...
                  <>
                    ...
                    <BlockStack gap="200">
                      <InlineStack align="space-between">
                        ...
                        <Badge tone="success">
                          {subscription.isTrialing ? t("billing.trialing") : t("billing.active")}
                        </Badge>
                      </InlineStack>
                      {subscription.currentPeriodEnd && (
                        <InlineStack align="space-between">
                          ...
                          <Text as="span">{new Date(subscription.currentPeriodEnd).toLocaleDateString("zh-CN")}</Text>
                        </InlineStack>
                      )}
                    </BlockStack>
                    ...
                    {(subscription as typeof subscription & { status?: string }).status === "CANCELLED" &&
                      subscription.currentPeriodEnd && (
                        <Banner tone="info" title={t("billing.subscriptionCancelled")}>
                          <p>
                            {t("billing.cancelledMessage", {
                              date: new Date(subscription.currentPeriodEnd).toLocaleDateString("zh-CN"),
                            })}
                          </p>
                        </Banner>
                      )}
