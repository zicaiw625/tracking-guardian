...
export function useScriptAnalysis(scriptAnalysisMaxContentLength: number, scriptAnalysisChunkSize: number) {
      ...
      if (isLargeContent) {
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          await new Promise<void>((resolve) => {
            ...
            if (typeof window !== "undefined" && "requestIdleCallback" in window) {
              ...
              const handle = setTimeout(processChunk, TIMEOUTS.SET_TIMEOUT_FALLBACK) as unknown as
                | number
                | IdleCallbackHandle;
              idleCallbackHandlesRef.current.push(handle);
            }
        ...
        result = await new Promise<ScriptAnalysisResult>((resolve, reject) => {
          ...
          if (typeof window !== "undefined" && "requestIdleCallback" in window) {
            ...
            const handle = setTimeout(processContent, TIMEOUTS.SET_TIMEOUT_FALLBACK) as unknown as
              | number
              | IdleCallbackHandle;
            idleCallbackHandlesRef.current.push(handle);
          }
