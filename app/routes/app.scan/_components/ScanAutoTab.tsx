...
export function ScanAutoTab({
              ...
              <BlockStack gap="300">
                ...
                <InlineStack gap="400" align="space-between" wrap>
                  <BlockStack gap="100">
                    ...
                    <Text as="span" variant="bodySm" tone="subdued">
                      {t("scan.autoTab.migrationProgress.stats", {
                        completed: migrationProgress.completed,
                        inProgress: migrationProgress.inProgress,
                        pending: migrationProgress.pending,
                      })}
                    </Text>
                  </BlockStack>
                </InlineStack>
  ...
  return (
    <BlockStack gap="500">
                    ...
                    <>
                                ...
                                <InlineStack gap="200">
                                  ...
                                  {(item.impact || item.impactKey) && (
                                    <Text as="span" variant="bodySm" tone="critical">
                                      {t("scan.riskDetails.impact")}{" "}
                                      {item.impactKey ? t(item.impactKey, item.impactParams) : item.impact}
                                    </Text>
                                  )}
                                </InlineStack>
                                <Button
                      ...
                      {isFreePlan && hiddenCount > 0 && (
                        <Banner tone="warning">
                          <BlockStack gap="200">
                            <Text as="p" variant="bodySm">
                              <strong>{t("scan.riskDetails.freeLimit")}</strong>
                              {t("scan.riskDetails.freeLimitDesc", { limit: FREE_AUDIT_LIMIT, count: hiddenCount })}
                            </Text>
                            <InlineStack gap="200">
                              <Button url="/app/billing" variant="primary" size="slim">
                                {t("scan.riskDetails.upgradeUnlock")}
                              </Button>
                              <Button url="/app/migrate" size="slim">
                      ...
                      <Box background="bg-surface-secondary" padding="400" borderRadius="200">
                        <BlockStack gap="300">
                          ...
                          {isFreePlan && (
                            <Banner tone="info">
                              <Text as="p" variant="bodySm">
                                <strong>{t("scan.riskDetails.upgradeMigration")}</strong>
                                {t("scan.riskDetails.upgradeMigrationDesc")}
                              </Text>
                            </Banner>
                          )}
                        </BlockStack>
              ...
              <BlockStack gap="400">
                <InlineStack align="space-between" blockAlign="center">
                  ...
                  <Badge tone="attention">
                    {t("scan.migrationActionsCard.pending", { count: migrationActions.length })}
                  </Badge>
                </InlineStack>
                <BlockStack gap="300">
                  {migrationActions.map((action, index) => (
                    <Box
                      key={`${action.type}-${action.platform || "unknown"}-${action.scriptTagId || action.webPixelGid || index}`}
                      ...
                      <BlockStack gap="300">
                        <InlineStack align="space-between" blockAlign="start">
                          <BlockStack gap="100">
                            <InlineStack gap="200" blockAlign="center">
                              ...
                              >
                                {action.priority === "high"
                                  ? t("scan.autoTab.upgradeWindow.high")
                                  : action.priority === "medium"
                                    ? t("scan.autoTab.upgradeWindow.medium")
                                    : t("scan.autoTab.upgradeWindow.low")}
                              </Badge>
                            </InlineStack>
                        ...
                        <Text as="p" variant="bodySm" tone="subdued">
                          {action.descriptionKey
                            ? t(action.descriptionKey, action.descriptionParams)
                            : action.description}
                        </Text>
          ...
          {auditAssets && Array.isArray(auditAssets) && auditAssets.length > 0 && (
            <AuditAssetsByRisk
              ...
              currentPlan={
                planId === "pro"
                  ? "growth"
                  : planId === "free" || planId === "starter" || planId === "growth" || planId === "agency"
                    ? planId
                    : "free"
              }
              freeTierLimit={3}
                    ...
                    {migrationTimeline.totalEstimatedTime > 0 && (
                      <InlineStack gap="200" blockAlign="center">
                        ...
                        <Text as="span" variant="bodySm" tone="subdued" fontWeight="semibold">
                          {t("scan.autoTab.migrationProgress.remainingTime")}{" "}
                          {Math.round(migrationTimeline.totalEstimatedTime / 60)} {t("common.hours")}{" "}
                          {migrationTimeline.totalEstimatedTime % 60} {t("common.minutes")}
                        </Text>
                      </InlineStack>
                    )}
                  </InlineStack>
                ...
                {migrationTimeline.assets.length > 0 && (
                  <>
                    ...
                    <BlockStack gap="300">
                          ...
                          <Box key={item.asset.id} background="bg-surface-secondary" padding="300" borderRadius="200">
                            <InlineStack align="space-between" blockAlign="center">
                              <BlockStack gap="100">
                                <InlineStack gap="200" blockAlign="center">
                                  <Text as="span" fontWeight="semibold">
                                    {item.asset.displayName ||
                                      item.asset.platform ||
                                      t("scan.autoTab.migrationProgress.unknownAsset")}
                                  </Text>
                                  <Badge
                                    tone={
                                      (item.asset.priority || item.priority.priority) >= 8
                                        ? "critical"
                                        : (item.asset.priority || item.priority.priority) >= 5
                                          ? undefined
                                          : "info"
                                    }
                                  >
                                    {t("scan.autoTab.migrationProgress.priority", {
                                      priority: item.asset.priority || item.priority.priority,
                                    })}
                                  </Badge>
                                  {(item.asset.priority || item.priority.priority) >= 8 && (
                                ...
                                <InlineStack gap="200" blockAlign="center">
                                  <Text as="span" variant="bodySm" tone="subdued">
                                    {item.priority.reasonKey
                                      ? t(item.priority.reasonKey, item.priority.reasonParams)
                                      : item.priority.reason || t("scan.autoTab.migrationProgress.noReason")}
                                  </Text>
                                  {item.asset.estimatedTimeMinutes && (
                                    <InlineStack gap="100" blockAlign="center">
                                      ...
                                      <Badge>
                                        {`${t("common.estimated")} ${
                                          item.asset.estimatedTimeMinutes < 60
                                  ...
                                  {!item.asset.estimatedTimeMinutes && item.priority.estimatedTime && (
                                    <InlineStack gap="100" blockAlign="center">
                                      ...
                                      <Badge>
                                        {`${t("common.estimated")} ${
                                          item.priority.estimatedTime < 60
                                ...
                                {item.blockingDependencies.length > 0 && (
                                  <Banner tone="warning">
                                    <Text as="p" variant="bodySm">
                                      {t("scan.autoTab.migrationProgress.waitingDependencies", {
                                        count: item.blockingDependencies.length,
                                      })}
                                    </Text>
                                  </Banner>
                                )}
                      ...
                      {migrationTimeline.assets.filter(
                        (item: any) => item.canStart && item.asset.migrationStatus === "pending"
                      ).length === 0 && (
                        <Banner tone="success">
                          <Text as="p" variant="bodySm">
                ...
                <Box background="bg-surface-secondary" padding="400" borderRadius="200">
                  <BlockStack gap="200">
                    <Text as="p" fontWeight="semibold">
                      {t("scan.autoTab.wizard.pendingItems")}
                    </Text>
                    <List type="number">
                      {migrationActions && migrationActions.length > 0 ? (
                        migrationActions.slice(0, MAX_VISIBLE_ACTIONS).map((action, index) => (
                          <List.Item
                            key={`${action.type}-${action.platform || "unknown"}-${action.scriptTagId || action.webPixelGid || "no-id"}-${index}`}
                          >
                            {action.titleKey ? t(action.titleKey, action.titleParams) : action.title}
                      ...
                      {migrationActions && migrationActions.length > MAX_VISIBLE_ACTIONS && (
                        <List.Item>
                          {t("scan.autoTab.migrationActions.moreItems", {
                            count: migrationActions.length - MAX_VISIBLE_ACTIONS,
                          })}
                        </List.Item>
                      )}
                    </List>
                    <InlineStack gap="200" align="end">
                      <Button icon={ClipboardIcon} loading={isCopying} onClick={onCopyChecklist}>
                        {t("scan.autoTab.wizard.copyChecklist")}
                      </Button>
                      <Button icon={ExportIcon} loading={isExporting} onClick={onExportChecklist}>
                    ...
                    <InlineStack gap="400" wrap>
                      <Box minWidth="200px">
                        <BlockStack gap="100">
                          ...
                          <Text as="p" variant="bodySm">
                            <Trans
                              i18nKey="scan.autoTab.wizard.officialAlternativeDesc"
                      ...
                      <Box minWidth="200px">
                        <BlockStack gap="100">
                          ...
                          <Text as="p" variant="bodySm">
                            <Trans
                              i18nKey="scan.autoTab.wizard.webPixelAlternativeDesc"
                              components={{ strong: <strong />, a: <a target="_blank" rel="noopener noreferrer" /> }}
                            />
                          </Text>
                        </BlockStack>
                      ...
                      <Box minWidth="200px">
                        <BlockStack gap="100">
                          ...
                          <Text as="p" variant="bodySm">
                            <Trans
                              i18nKey="scan.autoTab.wizard.pageCustomizationDesc"
                              components={{ strong: <strong />, a: <a target="_blank" rel="noopener noreferrer" /> }}
                            />
                          </Text>
                          <Text as="p" variant="bodySm" tone="subdued">
                            <strong>{t("scan.autoTab.wizard.pageCustomizationNote")}</strong>
                            {t("scan.autoTab.wizard.pageCustomizationNoteDesc")}
                          </Text>
                        </BlockStack>
                      </Box>
                    </InlineStack>
      ...
      {scanHistory && scanHistory.length > 0 && <ScanHistoryTable scanHistory={scanHistory} onStartScan={handleScan} />}

      {latestScan && (latestScan.riskScore || 0) > 0 && (
        <Banner
          title={t("scan.autoTab.suggestMigrationBanner.title")}
          tone="warning"
          action={{ content: t("scan.autoTab.suggestMigrationBanner.action"), url: "/app/migrate" }}
        >
          <p>{t("scan.autoTab.suggestMigrationBanner.content")}</p>
        </Banner>
      )}
    </BlockStack>
  );
