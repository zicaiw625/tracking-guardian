...
export const loader = async ({ request }: LoaderFunctionArgs) => {
      ...
      try {
        ...
        if (Array.isArray(pixels)) {
          webPixels = pixels
            ...
            .map((p) => ({
              id: p.id,
              settings: p.settings !== undefined && typeof p.settings === "string" ? p.settings : null,
            }));
  ...
  const scriptTags: ScriptTag[] = latestScan ? validateScriptTagsArray(latestScan.scriptTags) : [];
  ...
  const [typOspResult, migrationTimeline, migrationProgress, dependencyGraph, auditAssets, migrationChecklist] =
    await Promise.all([
      typOspPromise,
      generateMigrationTimeline(shop.id).catch((error) => {
        ...
        logger.error(
          "Failed to generate migration timeline",
      ...
      generateMigrationChecklist(shop.id).catch((error) => {
        ...
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger.error(
          "Failed to generate migration checklist",
          error instanceof Error ? error : new Error(String(error)),
          {
            shopId: shop.id,
            errorMessage,
