...
const AnalysisDataSchema = z.object({
  identifiedPlatforms: z
    .array(
      z
        .string()
        .min(SAVE_ANALYSIS_LIMITS.MIN_PLATFORM_NAME_LENGTH)
        .max(SAVE_ANALYSIS_LIMITS.MAX_PLATFORM_NAME_LENGTH)
        .regex(PLATFORM_NAME_REGEX)
    )
  ...
  recommendations: z
    .array(z.string().min(1).max(SAVE_ANALYSIS_LIMITS.MAX_RECOMMENDATION_LENGTH))
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (actionType === "save_analysis") {
    try {
      ...
      const risks = Array.isArray(data.risks)
        ...
        : [];
      const riskScore =
        typeof data.riskScore === "number" && !isNaN(data.riskScore) ? Math.max(0, Math.min(100, data.riskScore)) : 0;
  ...
  if (actionType === "export_checklist_csv") {
    try {
      ...
      csvLines.push(
        `预计总时间: ${Math.floor(checklist.estimatedTotalTime / 60)} 小时 ${checklist.estimatedTotalTime % 60} 分钟`
      );
      csvLines.push("");
      ...
      checklist.items.forEach((item) => {
        ...
        const row = [assetName, riskDisplay, migrationPath, timeAndInfo];
