        ...
        <Layout.Section>
          {extensionConfigStatus === "placeholder" && (
            <Banner tone="critical" title="Extension Config Warning">
              <p>
                <code>extensions/shared/config.ts</code> contains <code>__BACKEND_URL_PLACEHOLDER__</code>. You must run{" "}
                <code>pnpm ext:inject</code> before deploying to ensure the correct Backend URL is used.
              </p>
            </Banner>
          )}
          {extensionConfigStatus === "error" && (
            <Banner tone="warning" title="Extension Config Check Failed">
              <p>
                Could not read <code>extensions/shared/config.ts</code>. Please check file permissions.
              </p>
            </Banner>
          )}
            ...
            <BlockStack gap="400">
              ...
              <List>
                ...
                <List.Item>
                  ...
                  {isLocal ? "本地开发 (Localhost)" : "生产环境"}
                  {isLocal && (
                    <Text as="span" tone="critical">
                      {" "}
                      (注意：Localhost URL 无法在真实 Web Pixel 环境中工作)
                    </Text>
                  )}
                </List.Item>
                <List.Item>
                  ...
                  {extensionConfigStatus === "injected" ? "✅ Injected" : "⚠️ Placeholder/Error"}
                </List.Item>
              </List>

              <Banner tone="info" title="Network Access Allowlist Required">
                <p>
                  Ensure the following URL is added to your{" "}
                  <strong>Partner Dashboard &gt; App &gt; Extensions &gt; Web Pixel &gt; Network access</strong>:
                </p>
          ...
          <Card>
            <BlockStack gap="400">
              ...
              <Banner tone={corsStatus === "success" ? "success" : corsStatus === "error" ? "critical" : "info"}>
                {corsMessage}
              </Banner>
              <Button onClick={checkCors} loading={corsStatus === "pending"}>
                重新测试
              </Button>

              <Text as="p" variant="bodySm" tone="subdued">
                此测试模拟从浏览器（类似于 Web Pixel 环境）向后端发送跨域请求。如果失败，说明 CORS
                配置有误或后端不可达。 请确保 `extensions/tracking-pixel/src/index.ts` 中的 `BACKEND_URL` 与上述 URL
                一致，且 Shopify App URL 已配置为 HTTPS。
              </Text>
            </BlockStack>
          </Card>
        </Layout.Section>
