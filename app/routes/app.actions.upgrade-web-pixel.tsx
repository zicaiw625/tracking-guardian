...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    for (const pixel of webPixels) {
      ...
      try {
        const settings = typeof pixel.settings === "string" ? JSON.parse(pixel.settings) : pixel.settings;
    ...
    for (const pixel of pixelsToUpgrade) {
      const result = await upgradeWebPixelSettings(admin, pixel.id, pixel.settings, shopDomain, ingestionKey);
      results.push({
        pixelId: pixel.id,
