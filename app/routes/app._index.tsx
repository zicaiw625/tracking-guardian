...
import { Page, Layout, Card, Text, BlockStack, InlineStack, Badge, Button, Icon, Box, Divider, Banner, Link, ProgressBar, } from "@shopify/polaris";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    if (recentReports.length > 0 && configuredPlatforms > 0) {
        const avgDiscrepancy = recentReports.reduce((sum, r) => sum + r.orderDiscrepancy, 0) /
    ...
    if (latestScan?.scriptTags) {
        const scriptTags = latestScan.scriptTags as Array<{
...
export default function Index() {
    const { shopDomain, healthScore, healthStatus, latestScan, configuredPlatforms, weeklyConversions, hasAlertConfig, plan, scriptTagsCount, hasOrderStatusScripts, } = useLoaderData<typeof loader>();
    const setupSteps = [
        {
            id: "scan",
            label: "运行一次扫描",
    ...
    return (<Page title="Tracking Guardian" primaryAction={!allStepsCompleted && nextStep
            ? {
      ...
      <BlockStack gap="500">
        ...
        {!allStepsCompleted && (<Card>
            <BlockStack gap="400">
              ...
              <ProgressBar progress={(completedSteps / setupSteps.length) * 100} tone="primary" size="small"/>
              <BlockStack gap="300">
                {setupSteps.map((step, index) => (<Box key={step.id} background={step.done ? "bg-surface-success" : "bg-surface-secondary"} padding="400" borderRadius="200">
                    <InlineStack align="space-between" blockAlign="center">
                      <InlineStack gap="300" blockAlign="center">
                        <Box>
                          {step.done ? (<Icon source={CheckCircleIcon} tone="success"/>) : (<Text as="span" variant="bodyMd" fontWeight="bold">
                              {index + 1}
                            </Text>)}
                        </Box>
                        <BlockStack gap="100">
                      ...
                      {!step.done && (<Button url={step.url} size="slim" variant={step.id === nextStep?.id ? "primary" : undefined}>
                          {step.cta}
                        </Button>)}
                    </InlineStack>
                  </Box>))}
          ...
          <Layout.Section variant="oneThird">
            <Card>
              <BlockStack gap="400">
                ...
                <Box background={healthScore === null
            ...
            <Card>
              <BlockStack gap="400">
                <InlineStack align="space-between">
                  ...
                  {latestScan && (<Badge tone={latestScan.riskScore > 60
                ? "critical"
                ...
                {latestScan ? (<BlockStack gap="200">
                    <Text as="p" variant="bodySm" tone="subdued">
                      扫描时间:{" "}
                      {new Date(latestScan.createdAt).toLocaleDateString("zh-CN")}
                    </Text>
                    <Text as="p" variant="bodySm">
                      识别到的平台:{" "}
                      {((latestScan.identifiedPlatforms as string[]) || []).join(", ") || "无"}
                    </Text>
                  </BlockStack>) : (<Text as="p" tone="subdued">
                    尚未进行扫描
                  </Text>)}
                {latestScan ? (<Button url="/app/scan" fullWidth>
                    查看扫描报告
                  </Button>) : (<BlockStack gap="100">
                    <Text as="p" variant="bodySm" tone="subdued">
                      完成上方第 1 步开始扫描
                    </Text>
                    <Text as="p" variant="bodySm" tone="subdued">
        ...
        {scriptTagsCount > 0 && (<Banner title={`检测到 ${scriptTagsCount} 个 ScriptTag 需要迁移`} tone="critical" action={{
                content: "查看迁移方案",
                url: "/app/migrate",
            }} secondaryAction={{
                content: "查看扫描详情",
            ...
            <BlockStack gap="300">
              {hasOrderStatusScripts && (<Text as="p">
        ...
        <Banner title="重要迁移截止日期" tone={scriptTagsCount > 0 ? "warning" : "info"} action={{
            content: "了解更多",
          ...
          <BlockStack gap="300">
            <BlockStack gap="100">
              ...
              <Text as="p" variant="bodySm" tone="subdued">
                <Link url="https://help.shopify.com/en/manual/checkout-settings/customize-checkout-configurations/upgrade-thank-you-order-status/plus-upgrade-guide" external>
                  查看 Plus 商家升级指南
                </Link>
              </Text>
            </BlockStack>
            <BlockStack gap="100">
              ...
              <Text as="p" variant="bodySm" tone="subdued">
                <Link url="https://shopify.dev/docs/apps/build/online-store/blocking-script-tags" external>
                  查看 ScriptTags 弃用时间表
                </Link>
              </Text>
            </BlockStack>
