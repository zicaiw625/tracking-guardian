...
const ScriptCodeEditor = lazy(() =>
  import("~/components/scan/ScriptCodeEditor").then((module) => ({ default: module.ScriptCodeEditor }))
);
const MigrationChecklistTab = lazy(() =>
  import("./app.scan/_components/MigrationChecklistTab").then((m) => ({ default: m.MigrationChecklistTab }))
);
const ScanAutoTab = lazy(() => import("./app.scan/_components/ScanAutoTab").then((m) => ({ default: m.ScanAutoTab })));
const ScanManualSupplementTab = lazy(() =>
  import("./app.scan/_components/ScanManualSupplementTab").then((m) => ({ default: m.ScanManualSupplementTab }))
);
import { isPlanAtLeast } from "../utils/plans";
import { validateScriptTagsArray, validateRiskItemsArray, validateStringArray } from "../utils/scan-data-validation";
...
export function ScanPage({
  ...
  const [replacementChecklistItems, setReplacementChecklistItems] = useState<
    Array<{ id: string; contentSummary: string; result: ScriptAnalysisResult }>
  >([]);
  ...
  const introConfig = useMemo(() => {
    if (selectedTab === 1) {
      return {
        ...
        items: [t("scan.intro.manual.items.0"), t("scan.intro.manual.items.1"), t("scan.intro.manual.items.2")],
    ...
    return {
      ...
      items: [t("scan.intro.auto.items.0"), t("scan.intro.auto.items.1"), t("scan.intro.auto.items.2")],
  ...
  const handleShowScriptTagGuidance = useCallback(
    (scriptTagId: number, platform?: string) => {
      setGuidanceContent({
  ...
  const exportReplacementChecklistCSV = useCallback(() => {
    ...
    const getReplacementSuggestion = (r: ScriptAnalysisResult): string => {
      ...
      const hasDomRisk = r.risks.some(
        (risk) => /window|document|dom/i.test(risk.id) || /window|document|dom/i.test(risk.name || "")
      );
    ...
    const rows = replacementChecklistItems.map((item, i) => {
      ...
      return [i + 1, item.contentSummary, platforms, repl, item.result.riskScore, topRisk, recs]
        .map(escapeCSV)
        .join(",");
    });
  ...
  const handleManualInputComplete = useCallback(
  ...
  const auditAssetCount = useMemo(
    () =>
      Array.isArray(auditAssets)
        ? auditAssets.filter((asset): asset is NonNullable<typeof asset> => asset !== null).length
        : 0,
    [auditAssets]
  );
  useEffect(() => {
  ...
  const handleGenerateChecklistText = useCallback(
  ...
  const handleExportCSV = useCallback(async () => {
    ...
    try {
      ...
      showError(
        t("scan.errors.exportFailed") + "：" + (error instanceof Error ? error.message : t("common.unknownError"))
      );
    }
  ...
  return (
    <Page title={pageTitle || t("scan.pageTitle")} subtitle={pageSubtitle || t("scan.pageSubtitle")}>
      <BlockStack gap="500">
        <ScanPageBanners
        ...
        <Tabs tabs={visibleTabs} selected={selectedTab} onSelect={setSelectedTab}>
          {}
          {shouldShowMigrationButtons && <AuditPaywallCard planId={planIdSafe} />}
          {selectedTab === 0 && (
            <Suspense
              fallback={
                <Card>
                  <BlockStack gap="400">
                    <CardSkeleton lines={4} showTitle />
                  </BlockStack>
                </Card>
              }
            >
