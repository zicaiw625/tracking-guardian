...
import { tryAuthenticatePublicWithShop, handlePublicPreflight, addSecurityHeaders, publicJsonWithAuthCors } from "../../utils/public-auth";
...
import { API_SECURITY_HEADERS, addSecurityHeadersToHeaders } from "../../utils/security-headers";

export const action = async ({ request }: ActionFunctionArgs) => {
  if (request.method === "OPTIONS") {
    return handlePublicPreflight(request);
  }
  const wrapUnauthenticated = (r: Response) => {
  ...
  if (request.method !== "POST") {
    return wrapUnauthenticated(json({ error: "Method not allowed" }, { status: 405 }));
  }
  const ct = request.headers.get("Content-Type") || "";
  if (!/^application\/json($|;)/.test(ct)) {
    return wrapUnauthenticated(json({ error: "Content-Type must be application/json" }, { status: 415 }));
  }
  const auth = await tryAuthenticatePublicWithShop(request);
  if (!auth) {
    return wrapUnauthenticated(json({ error: "Unauthorized: Invalid authentication" }, { status: 401 }));
  }
  ...
  if (!shop) {
    ...
    return publicJsonWithAuthCors(request, authResult, { error: "Shop not found" }, { status: 404 });
  }
  if (shop.isActive === false) {
    return publicJsonWithAuthCors(request, authResult, { error: "Shop is not active" }, { status: 403 });
  }
  ...
  if (!moduleCheck.allowed) {
    ...
    return publicJsonWithAuthCors(request, authResult, { error: "Module not available", reason: moduleCheck.reason }, { status: 403 });
  }
  ...
  if (!surveyModule || !surveyModule.isEnabled) {
    ...
    return publicJsonWithAuthCors(request, authResult, { error: "Survey module is not enabled" }, { status: 403 });
  }
  ...
  try {
    ...
    if (!body || typeof body !== "object") {
      return publicJsonWithAuthCors(request, authResult, { error: "Invalid request body" }, { status: 400 });
    }
    ...
    if (!option) {
      return publicJsonWithAuthCors(request, authResult, { error: "Missing survey option" }, { status: 400 });
    }
    const trimmed = option.trim();
    if (trimmed.length === 0 || trimmed.length > 200) {
      return publicJsonWithAuthCors(request, authResult, { error: "Invalid option length" }, { status: 400 });
    }
    if (containsSensitiveInfo(trimmed)) {
      return publicJsonWithAuthCors(request, authResult, { error: "Option contains sensitive info" }, { status: 400 });
    }
    const safeOption = sanitizeSensitiveInfo(trimmed);
    const orderKey = makeOrderKey({ orderId, checkoutToken });
    if (!orderKey) {
      return publicJsonWithAuthCors(request, authResult, { error: "Order context required (orderId or checkoutToken). Unavailable when PCD is not approved." }, { status: 400 });
    }
    if (orderId) {
    ...
    if (timestamp !== undefined && timestamp !== null && typeof timestamp !== "string") {
      return publicJsonWithAuthCors(request, authResult, { error: "Invalid timestamp" }, { status: 400 });
    }
    ...
    if (existing) {
      ...
      return publicJsonWithAuthCors(request, authResult, { success: true, message: "Survey response updated", id: existing.id });
    }
    ...
    logger.info("Survey response received", { shopDomain });
    return publicJsonWithAuthCors(request, authResult, { success: true, message: "Survey response recorded", id: created.id });
  } catch (error) {
    if (error instanceof Response) {
      const corsResponse = authResult.cors(error);
      const mergedHeaders = new Headers(corsResponse.headers);
      const dynamicCors = getDynamicCorsHeaders(request, ["Authorization"]);
    ...
    return publicJsonWithAuthCors(request, authResult, { error: "Internal server error" }, { status: 500 });
  }
};
