...
export function MigrationDependencyGraph({
  ...
  function formatTime(minutes: number): string {
    if (minutes < 60) {
      return t("migrationDependency.time.minutes", { minutes });
    }
    const hours = Math.floor(minutes / 60);
    const mins = minutes % 60;
    return mins > 0 ? t("migrationDependency.time.hoursAndMinutes", { hours, mins }) : t("migrationDependency.time.hours", { hours });
  }
      ...
      <Card>
        <BlockStack gap="300">
          <Text as="h3" variant="headingMd">
            {t("migrationDependency.title")}
          </Text>
          <Banner>
            <Text as="p" variant="bodySm">
              {t("migrationDependency.empty")}
            </Text>
          </Banner>
        </BlockStack>
      </Card>
    ...
    <Card>
      <BlockStack gap="400">
        <InlineStack align="space-between" blockAlign="center">
          ...
          <Badge tone="info">
            {t("migrationDependency.assetCount", { count: dependencyGraph.nodes.length })}
          </Badge>
        </InlineStack>
        {cycles.length > 0 && (
          <Banner tone="critical">
            <BlockStack gap="200">
              <Text as="p" variant="bodySm" fontWeight="semibold">
                {t("migrationDependency.cyclicDependency.title")}
              </Text>
              <Text as="p" variant="bodySm">
                {t("migrationDependency.cyclicDependency.description")}
              </Text>
              <List type="bullet">
                {cycles.map((cycle, index) => (
                  <List.Item key={index}>
                    <Text as="span" variant="bodySm">
                      {t("migrationDependency.cyclicDependency.cycleCount", { count: cycle.length })}
                    </Text>
                  </List.Item>
                ))}
              </List>
            ...
            <BlockStack gap="300">
              <InlineStack align="space-between" blockAlign="center">
                <Text as="h4" variant="headingSm">
                  {t("migrationDependency.criticalPath.title")}
                </Text>
                <Badge>
                  {t("migrationDependency.criticalPath.steps", { count: criticalPath.length })}
                </Badge>
              </InlineStack>
              <List type="number">
                  ...
                  return (
                    <List.Item key={nodeId}>
                      <InlineStack gap="200" blockAlign="center" wrap>
                        ...
                        {node.priority && (
                          <Badge tone="success">
                            {t("migrationDependency.priority", { priority: node.priority })}
                          </Badge>
                        )}
                        {node.estimatedTimeMinutes && (
                          <Badge tone="info">
                            {t("migrationDependency.estimatedTime", { time: formatTime(node.estimatedTimeMinutes) })}
                          </Badge>
                        )}
                      </InlineStack>
                    </List.Item>
        ...
        >
          <BlockStack gap="300">
            <Text as="h4" variant="headingSm">
              {t("migrationDependency.recommendedOrder")}
            </Text>
            <List type="number">
                      ...
                      {dependencies.length > 0 && (
                        <Box paddingInlineStart="400">
                          <Text as="p" variant="bodySm" tone="subdued">
                            {t("migrationDependency.dependencyCount", { count: dependencies.length })}
                          </Text>
                        </Box>
                      )}
                    </BlockStack>
        ...
        >
          <BlockStack gap="300">
            <Text as="h4" variant="headingSm">
              {t("migrationDependency.dependencyDetails")}
            </Text>
