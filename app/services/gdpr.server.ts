...
async function processDataRequest(shopDomain: string, payload: DataRequestPayload): Promise<DataRequestResult> {
    ...
    const exportedPixelReceipts: ExportedPixelEventReceipt[] = pixelReceipts.map(receipt => ({
        ...
        consentState: receipt.consentState as {
            marketing?: boolean;
            analytics?: boolean;
        } | null,
...
async function processCustomerRedact(shopDomain: string, payload: CustomerRedactPayload): Promise<CustomerRedactResult> {
    const customerId = payload.customer_id;
    ...
    const linkedCheckoutTokens = Array.from(new Set(receiptsWithCheckoutTokens
...
async function processShopRedact(shopDomain: string, _payload: ShopRedactPayload): Promise<ShopRedactResult> {
...
export async function checkGDPRCompliance(): Promise<{
    ...
    for (const job of pendingJobs) {
        ...
        if (job.createdAt < thirtyDaysAgo) {
            overdueCount++;
            criticals.push(`[CRITICAL] GDPR ${job.jobType} for ${job.shopDomain} is ${ageDays} days old (> 30 day limit). Job ID: ${job.id}`);
        }
        else if (job.createdAt < sevenDaysAgo) {
            warnings.push(`[WARNING] GDPR ${job.jobType} for ${job.shopDomain} is ${ageDays} days old. Job ID: ${job.id}`);
        }
    }
...
export async function getGDPRDeletionSummary(startDate: Date, endDate: Date): Promise<{
