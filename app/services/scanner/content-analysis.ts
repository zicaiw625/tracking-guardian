...
export function analyzeScriptContent(content: string, t?: TFunction): ScriptAnalysisResult {
  ...
  if (snapPixelMatch) {
    for (const match of snapPixelMatch) {
      ...
      if (
        pixelId &&
        !result.platformDetails.some((d) => d.platform === "snapchat" && d.matchedPattern.includes(pixelId))
      ) {
        let cleanedPixelId = sanitizeSensitiveInfo(pixelId);
        if (cleanedPixelId.length > 50) {
          cleanedPixelId = cleanedPixelId.substring(0, 50) + "...";
  ...
  if (pinterestTagMatch) {
    for (const match of pinterestTagMatch) {
      ...
      if (
        tagId &&
        !result.platformDetails.some((d) => d.platform === "pinterest" && d.matchedPattern.includes(tagId))
      ) {
  ...
  if (tiktokPixelMatch) {
    for (const match of tiktokPixelMatch) {
      ...
      if (
        pixelId &&
        !result.platformDetails.some((d) => d.platform === "tiktok" && d.matchedPattern.includes(pixelId))
      ) {
        let cleanedPixelId = sanitizeSensitiveInfo(pixelId);
        if (cleanedPixelId.length > 50) {
          cleanedPixelId = cleanedPixelId.substring(0, 50) + "...";
  ...
  if (piiMatches.length > 0) {
    ...
    result.risks.push({
      ...
      description: getT(
        t,
        "scan.analysis.risks.pii_access.description",
        { types: typesStr },
        "脚本可能读取客户敏感信息"
      ),
      ...
      details: getT(
        t,
        "scan.analysis.risks.pii_access.details",
        { count: piiMatches.length, types: typesStr },
  ...
  if (windowDocumentMatches.length > 0) {
    ...
    result.risks.push({
      ...
      description: getT(
        t,
        "scan.analysis.risks.window_document_access.description",
        {},
      ...
      details: getT(
        t,
        "scan.analysis.risks.window_document_access.details",
        { count: uniqueMatches.length, issues: issuesStr },
  ...
  if (blockingMatches.length > 0) {
    ...
    result.risks.push({
      ...
      description: getT(
        t,
        "scan.analysis.risks.blocking_load.description",
        { types: typesStr },
        "脚本可能阻塞页面渲染"
      ),
      severity: "high" as RiskSeverity,
      ...
      details: getT(
        t,
        "scan.analysis.risks.blocking_load.details",
        { count: uniqueMatches.length, types: typesStr },
        `检测到 ${uniqueMatches.length} 处阻塞代码`
      ),
    });
  }

  const duplicatePatterns = [/(?:fbq|gtag|ttq|pintrk|snaptr)\s*\([^)]*['"](?:track|event|purchase|pageview)['"]/gi];
  ...
  if (result.identifiedPlatforms.length > 0) {
    ...
    result.risks.push({
      ...
      details: getT(
        t,
        "scan.analysis.risks.additional_scripts_detected.details",
    ...
    if (result.identifiedPlatforms.includes("google") && contentToAnalyze.includes("UA-")) {
      result.risks.push({
        ...
        description: getT(
          t,
          "scan.analysis.risks.legacy_ua.description",
          {},
          "Universal Analytics 已于 2023 年 7 月停止处理数据"
        ),
        severity: "high" as RiskSeverity,
        points: 30,
      });
  ...
  for (const platform of result.identifiedPlatforms) {
    ...
    if (!recommendation || recommendation === key) {
      recommendation = getT(
        t,
        "scan.analysis.recommendations.default",
        { platform },
        `请确认此 ${platform} 追踪代码的用途`
      );
    }
  ...
  if (result.identifiedPlatforms.length === 0 && contentToAnalyze.length > 100) {
    result.recommendations.push(getT(t, "scan.analysis.recommendations.unknown", {}, "未检测到已知追踪平台"));
  }

  if (result.identifiedPlatforms.length >= 2) {
    result.recommendations.push(getT(t, "scan.analysis.recommendations.checklist", {}, "迁移清单建议"));
  }
