...
import { batchCreateAuditAssets, type AuditAssetInput } from "../audit-asset.server";
import { sanitizeScriptTags } from "../../utils/url-sanitize.server";

export type { WebPixelInfo, EnhancedScanResult, MigrationAction, ScriptAnalysisResult } from "./types";
...
function isCheckoutConfig(value: unknown): value is CheckoutConfig {
  ...
  return typeof value.checkoutApiSupported === "boolean" || (value.features !== undefined && isRecord(value.features));
}
...
function detectDuplicatePixels(result: EnhancedScanResult): Array<{
  ...
  for (const tag of result.scriptTags) {
    ...
    if (tiktokMatch) {
      const hasTiktokContext =
        src.includes("tiktok") || src.includes("ttq") || src.includes("analytics.tiktok") || src.includes("tiktok.com");
      if (hasTiktokContext) {
    ...
    try {
      ...
      for (const [settingKey, value] of Object.entries(settings)) {
        ...
        if (/^G-[A-Z0-9]+$/.test(value)) {
          ...
          /^[A-Z0-9]{20,30}$/i.test(value) &&
          !value.includes(":") &&
          (settingKey.toLowerCase().includes("pixel") || settingKey.toLowerCase().includes("tiktok"))
        ) {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.warn(`Failed to parse pixel settings for pixel ${pixel.id} in detectDuplicatePixels:`, {
        error: errorMessage,
        pixelId: pixel.id,
      });
      continue;
  ...
  for (const [key, data] of Object.entries(platformIdentifiers)) {
    if (data.sources.length > 1) {
      ...
      logger.info(
        `Duplicate detected: ${platform} identifier ${identifier?.substring(0, 8)}... appears ${data.sources.length} times`
      );
    }
  }
  return duplicates;
...
export async function getCachedScanResult(
  ...
  return {
    ...
    _additionalScriptsNote:
      "Additional Scripts 需要通过手动粘贴识别，Shopify API 无法自动读取 checkout.liquid 中的 Additional Scripts 内容",
  };
}
...
export async function scanShopTracking(
  ...
  function safeJsonClone<T>(obj: T): T {
    try {
      ...
      logger.warn("Failed to clone object for database storage, using original:", {
  ...
  const [scriptTagsResult, checkoutConfigResult, webPixelsResult] = await Promise.allSettled([
    ...
    admin
      ...
      .then((res) => res.json())
      .then((data) => data.data?.shop as CheckoutConfig),
  ...
  if (checkoutConfigResult.status === "fulfilled") {
    ...
    const errorMessage =
      checkoutConfigResult.reason instanceof Error ? checkoutConfigResult.reason.message : "Unknown error";
    ...
    const savedReport = await prisma.scanReport.create({
      data: {
        ...
        checkoutConfig: result.checkoutConfig
          ? (safeJsonClone(result.checkoutConfig) as unknown as Prisma.InputJsonValue)
          : undefined,
  ...
  try {
    ...
    for (const tag of result.scriptTags) {
      ...
      if (riskDetection) {
        if (
          riskDetection.detectedIssues.piiAccess ||
      ...
      auditAssets.push({
        ...
        displayName: platform ? `ScriptTag: ${platform}` : `ScriptTag #${tag.id}`,
        riskLevel,
    ...
    for (const platform of result.identifiedPlatforms) {
      const hasScriptTag = result.scriptTags.some((tag) => detectPlatforms(tag.src || "").includes(platform));
      if (!hasScriptTag) {
        auditAssets.push({
          sourceType: "api_scan",
          category: "pixel",
