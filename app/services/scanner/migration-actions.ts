...
import { identifyPlatformFromSrc, getPlatformInfo } from "./patterns";
import {
...
export function calculateMigrationProgress(totalActions: MigrationAction[], completedActionIds: string[]): number {
...
export function generateMigrationActions(result: EnhancedScanResult, shopTier: string): MigrationAction[] {
  ...
  for (const dup of result.duplicatePixels) {
    ...
    const duplicateAction: MigrationAction = {
      ...
      description:
        `检测到 ${dup.count} 个 ${dup.platform} 像素配置，可能导致重复追踪。建议只保留一个。` +
        (gidsToDelete.length > 0 ? ` (可删除 ${gidsToDelete.length} 个)` : ""),
      descriptionKey:
        gidsToDelete.length > 0 ? "scan.migrationLogic.duplicate.descWithDelete" : "scan.migrationLogic.duplicate.desc",
  ...
  const hasAppPixelConfigured = result.webPixels.some((p) => {
    ...
    try {
      ...
      logger.warn(`Failed to parse pixel settings for pixel ${p.id} in hasAppPixelConfigured:`, {
  ...
  const pixelNeedsUpgrade = result.webPixels.some((p) => {
    ...
    try {
      ...
      logger.warn(`Failed to parse pixel settings for pixel ${p.id} in pixelNeedsUpgrade:`, {
        error: error instanceof Error ? error.message : String(error),
        pixelId: p.id,
      });
      return false;
  ...
  if (pixelNeedsUpgrade) {
    const upgradeAction: MigrationAction = {
      ...
      description:
        "Legacy Pixel configuration detected (missing shop_domain or using legacy ingestion_secret). Please re-enable App Pixel to upgrade to the new format.",
  ...
  const hasLegacyTracking =
    result.scriptTags.length > 0 || result.additionalScriptsPatterns.some((p) => p.platform !== "unknown");
  ...
  if (hasLegacyTracking && shopTier === "plus") {
    if (isInAutoUpgradeWindow) {
      const autoUpgradeAction: MigrationAction = {
        ...
        description:
          `Shopify has started automatically migrating Plus merchants to the new Thank you / Order status pages as of Jan 2026 (30-day notice provided). ` +
      ...
      const countdownAction: MigrationAction = {
        ...
        description:
          `Shopify will start automatically migrating Plus merchants to the new pages starting Jan 2026 (30-day notice provided). ` +
  ...
  for (const pixel of result.webPixels) {
    if (pixel.settings && typeof pixel.settings === "string") {
      try {
        ...
        logger.warn(`Failed to parse pixel settings for pixel ${pixel.id} in getConfiguredPlatforms:`, {
          error: error instanceof Error ? error.message : String(error),
          pixelId: pixel.id,
        });
        continue;
