...
export function assessRisks(result: EnhancedScanResult): RiskItem[] {
  ...
  if (result.scriptTags.length > 0) {
    const platformScriptTags: Record<
      string,
      {
        ...
        addRisk(
          {
            ...
            detailsParams: {
              ...
              scope: tags.other.map((t) => t.display_scope || "all").join(", "),
            },
            platform,
          },
    ...
    addRisk(
      {
        ...
        description:
          "Detected hardcoded tracking scripts in page source. Migration to Shopify Web Pixel is recommended. Detection method: URL pattern matching and content inference",
    ...
    addRisk(
      {
        ...
        description:
          "For detected supported platforms, enabling Conversion API is recommended to resist ad blockers and improve data accuracy",
