...
import { getScriptTagCreationStatus, getScriptTagExecutionStatus, getScriptTagDeprecationStatus, getAdditionalScriptsDeprecationStatus, type ShopTier } from "../utils/deprecation-dates";
...
function getPatternType(platform: string, pattern: RegExp): string {
    ...
    switch (platform) {
        case "google":
            if (patternStr.includes("gtag"))
                return "gtag() 函数调用";
            if (patternStr.includes("gtm"))
                return "Google Tag Manager";
            if (patternStr.includes("G-"))
                return "GA4 Measurement ID";
            if (patternStr.includes("AW-"))
                return "Google Ads Conversion ID";
            if (patternStr.includes("UA-"))
                return "Universal Analytics (已弃用)";
            return "Google 追踪代码";
        case "meta":
            if (patternStr.includes("fbq"))
                return "Meta Pixel 函数调用";
            if (patternStr.includes("facebook"))
                return "Facebook SDK";
            if (patternStr.includes("pixel"))
                return "Pixel ID";
            return "Meta 追踪代码";
        case "tiktok":
            if (patternStr.includes("ttq"))
                return "TikTok Pixel 函数调用";
            return "TikTok 追踪代码";
        case "bing":
            if (patternStr.includes("uet"))
                return "Microsoft UET 标签";
            return "Bing 追踪代码";
        case "clarity":
