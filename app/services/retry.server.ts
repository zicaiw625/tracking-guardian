...
import { type PlatformError, calculateBackoff, shouldRetry as shouldRetryPlatform, formatErrorForLog, } from "./platforms/base.server";
...
import { evaluatePlatformConsentWithStrategy, getEffectiveConsentCategory, type ConsentState, } from "../utils/platform-consent";
import { verifyReceiptTrust, isSendAllowedByTrust, buildTrustMetadata, buildShopAllowedDomains, type ReceiptTrustResult, } from "../utils/receipt-trust";
export type FailureReason = "token_expired" | "rate_limited" | "platform_error" | "validation_error" | "network_error" | "config_error" | "unknown";
...
export function shouldRetryFromPlatformError(error: PlatformError, currentAttempt: number, maxAttempts: number): boolean {
...
export function classifyFailureReason(errorMessage: string | null): FailureReason {
    ...
    if (lowerError.includes("400") ||
        lowerError.includes("invalid") ||
        lowerError.includes("validation") ||
        lowerError.includes("missing required")) {
        return "validation_error";
    }
    if (lowerError.includes("credential") ||
...
import type { ConversionData, GoogleCredentials, MetaCredentials, TikTokCredentials, PlatformCredentials, } from "../types";
interface DecryptCredentialsResult {
    credentials: PlatformCredentials | null;
    usedLegacy: boolean;
}
export function getDecryptedCredentials(pixelConfig: {
    credentialsEncrypted?: string | null;
} & Record<string, unknown>, platform: string): DecryptCredentialsResult {
...
export async function scheduleRetry(logId: string, errorMessage: string): Promise<{
    scheduled: boolean;
...
export async function processPendingConversions(): Promise<{
    ...
    for (const log of pendingLogs) {
        try {
            const billingCheck = await checkBillingGate(log.shopId, (log.shop.plan || "free") as PlanId);
            if (!billingCheck.allowed) {
                logger.info(`Billing gate blocked conversion ${log.id}: ${billingCheck.reason}, ` +
            ...
            const { credentials } = getDecryptedCredentials(pixelConfig as {
                credentialsEncrypted?: string | null;
            } & Record<string, unknown>, log.platform);
            if (!credentials) {
                await prisma.conversionLog.update({
                    where: { id: log.id },
                    data: {
            ...
            switch (log.platform) {
                case "google":
                    result = await sendConversionToGoogle(credentials as GoogleCredentials, conversionData, eventId);
                    break;
                case "meta":
                    result = await sendConversionToMeta(credentials as MetaCredentials, conversionData, eventId);
                    break;
                case "tiktok":
                    result = await sendConversionToTikTok(credentials as TikTokCredentials, conversionData, eventId);
                    break;
                default:
...
export async function processRetries(): Promise<{
    ...
    for (const log of logsToRetry) {
        try {
            const billingCheck = await checkBillingGate(log.shopId, (log.shop.plan || "free") as PlanId);
            if (!billingCheck.allowed) {
                logger.info(`Billing gate blocked retry for ${log.id}: ${billingCheck.reason}, ` +
                    `usage=${billingCheck.usage.current}/${billingCheck.usage.limit}`);
                await prisma.conversionLog.update({
                    where: { id: log.id },
                    data: {
...
async function claimJobsForProcessing(batchSize: number): Promise<string[]> {
    ...
    const claimedIds = await prisma.$transaction(async (tx) => {
        const availableJobs = await tx.$queryRaw<Array<{
            id: string;
        }>> `
...
export async function processConversionJobs(): Promise<{
    ...
    for (const job of jobsToProcess) {
        try {
            const billingCheck = await checkBillingGate(job.shopId, (job.shop.plan || "free") as PlanId);
            if (!billingCheck.allowed) {
                logger.info(`Billing gate blocked job ${job.id}: ${billingCheck.reason}, ` +
                    `usage=${billingCheck.usage.current}/${billingCheck.usage.limit}`);
            ...
            const capiInputRaw = job.capiInput as {
            ...
            if (!receipt && webhookCheckoutToken) {
                ...
                for (const candidate of potentialReceipts) {
                    if (matchKeysEqual({ orderId: job.orderId, checkoutToken: webhookCheckoutToken }, { orderId: candidate.orderId, checkoutToken: candidate.checkoutToken })) {
                        receipt = candidate;
                        break;
                    }
                }
            }
            const shopAllowedDomains = buildShopAllowedDomains(job.shop.shopDomain, job.shop.primaryDomain, job.shop.storefrontDomains);
            ...
            for (const pixelConfig of job.shop.pixelConfigs) {
                const clientConfig = pixelConfig.clientConfig as {
                ...
                if (consentState?.saleOfDataAllowed === false) {
                    logger.debug(`[P0-04] Skipping ${pixelConfig.platform} for job ${job.id}: ` +
                        `sale_of_data opt-out`);
                    platformResults[pixelConfig.platform] = "skipped:sale_of_data_opted_out";
                    continue;
                }
                const trustAllowed = isSendAllowedByTrust(trustResult, pixelConfig.platform, platformCategory, strategy);
                if (!trustAllowed.allowed) {
                    logger.debug(`[P0-01] Skipping ${pixelConfig.platform} for job ${job.id}: ` +
                ...
                const consentDecision = evaluatePlatformConsentWithStrategy(pixelConfig.platform, strategy, consentState as ConsentState | null, hasVerifiedReceipt, treatAsMarketing);
                if (!consentDecision.allowed) {
                    const skipReason = consentDecision.reason || "consent_denied";
                    const usedConsent = consentDecision.usedConsent || "unknown";
                    logger.debug(`[P0-07] Skipping ${pixelConfig.platform} for job ${job.id}: ` +
                ...
                logger.debug(`[P0-07] Consent check passed for ${pixelConfig.platform} (job ${job.id}): ` +
    ...
    logger.info(`processConversionJobs: Completed - ` +
...
export async function getDeadLetterItems(shopId: string, limit = 50): Promise<Array<{
    id: string;
    orderId: string;
