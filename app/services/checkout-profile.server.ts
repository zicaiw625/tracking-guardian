...
export async function getTypOspStatusFromShopFeatures(admin: AdminApiContext): Promise<TypOspStatusResult> {
  try {
    ...
    `);
    const data = (await response.json()) as {
      data?: { shop?: { checkoutApiSupported?: boolean; plan?: { shopifyPlus?: boolean } } };
      errors?: Array<{ message?: string }>;
    };
    if (data.errors) {
...
export async function refreshTypOspStatus(admin: AdminApiContext, shopId: string): Promise<TypOspStatusResult> {
  ...
  if (result.status === "unknown" && result.unknownReason !== "NO_ADMIN_CONTEXT") {
    ...
    if (fallbackResult.status === "enabled" || shouldAdoptDisabledFallback) {
...
export async function getCachedTypOspStatus(shopId: string): Promise<{
  ...
  const isStale = !lastChecked || Date.now() - lastChecked.getTime() > staleThreshold;
