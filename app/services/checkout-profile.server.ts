...
export type TypOspUnknownReason = "NOT_PLUS" | "NO_EDITOR_ACCESS" | "API_ERROR" | "RATE_LIMIT" | "NO_PROFILES" | "FIELD_NOT_AVAILABLE" | "NO_ADMIN_CONTEXT";
...
export async function refreshTypOspStatus(admin: AdminApiContext, shopId: string): Promise<TypOspStatusResult> {
    ...
    if (result.status === "unknown" && result.unknownReason !== "NO_ADMIN_CONTEXT") {
        ...
        const shouldAdoptDisabledFallback = fallbackResult.status === "disabled" &&
            result.unknownReason !== "NOT_PLUS" &&
            result.unknownReason !== "NO_EDITOR_ACCESS";
        if (fallbackResult.status === "enabled" ||
...
export async function refreshTypOspStatusWithOfflineToken(shopId: string, shopDomain: string): Promise<TypOspStatusResult> {
