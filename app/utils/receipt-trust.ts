...
import {
  ...
  extractOriginHost as extractOriginHostFromValidation,
  buildShopAllowedDomains as buildShopAllowedDomainsFromValidation,
...
export function verifyReceiptTrust(options: VerifyReceiptOptions): ReceiptTrustResult {
  ...
  const hasReceiptTokenSignal =
    isUsableTokenSignal(receiptCheckoutToken) || isUsableTokenSignal(receiptCheckoutTokenHash);
  ...
  const hasWebhookTokenSignal =
    isUsableTokenSignal(webhookCheckoutToken) || isUsableTokenSignal(webhookCheckoutTokenHash);
