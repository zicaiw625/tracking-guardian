...
export async function calculateMigrationProgress(shopId: string): Promise<MigrationProgress> {
  ...
  const scanReports =
    shop && "ScanReports" in shop ? (shop as typeof shop & { ScanReports: Array<{ status: string }> }).ScanReports : [];
  ...
  const pixelConfigs =
    shop && "pixelConfigs" in shop
      ? (shop as typeof shop & { pixelConfigs: Array<{ environment: string }> }).pixelConfigs
      : [];
  ...
  const verificationRuns =
    shop && "VerificationRun" in shop
      ? (shop as typeof shop & { VerificationRun: Array<{ status: string; completedAt: Date | null }> }).VerificationRun
      : [];
  ...
  const pixelConfigsCount =
    shop && "_count" in shop ? (shop as typeof shop & { _count: { pixelConfigs: number } })._count.pixelConfigs : 0;
  ...
  const auditCompletion = {
    ...
    status:
      auditStatus === "completed"
        ? ("completed" as const)
        : auditStatus === "scanning" || auditStatus === "pending"
  ...
  const verificationLatest = verificationRuns[0]
    ? {
        status:
          verificationRuns[0].status === "completed"
            ? ("completed" as const)
            : verificationRuns[0].status === "running"
              ? ("running" as const)
              : ("pending" as const),
