...
export function validatePixelOriginPreBody(
  origin: string | null,
  hasSignatureHeaderOrHMAC: boolean = false,
  originHeaderPresent: boolean = true
): {
  valid: boolean;
  reason: string;
  shouldLog: boolean;
  ...
  if (origin === "null") {
    if (!devMode && allowNullOrigin && !hasSignatureHeaderOrHMAC) {
      return {
        valid: false,
  ...
  if (origin === null) {
    if (!originHeaderPresent) {
      ...
      if (!hasSignatureHeaderOrHMAC) {
        return {
          valid: false,
          reason: "missing_origin_requires_signature",
          shouldLog: true,
          shouldReject: true,
        };
      }
  ...
  try {
    ...
    if (url.protocol === "https:") {
      ...
      const isShopifyDomain = SHOPIFY_PLATFORM_HOSTS.some(
    ...
    if (options?.referer) {
      try {
        ...
        const isRefererAllowed =
          shopAllowedDomains.some((domain) => {
  ...
  try {
    ...
    if (originSource !== "origin_header") {
      ...
      if (devMode) {
        ...
        const securityNote =
          originSource === "referer_header" && hasSignatureHeaderOrHMAC
...
function evictOldestEntries(count: number): void {
  ...
  const entries = Array.from(rejectedOrigins.entries()).sort((a, b) => a[1].lastSeen - b[1].lastSeen);
  ...
  return [
    {
      ...
      description: nullOriginAllowed
        ? "Web Pixel sandbox (allowed when request is signed)"
        : "Web Pixel sandbox (blocked by configuration)",
      example: "Origin: null",
    },
