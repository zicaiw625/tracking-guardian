...
function shouldForceProductionNodeEnv(): boolean {
...
export function checkSecurityViolations(): SecurityViolation[] {
  ...
  if (shouldForceProductionNodeEnv() && process.env.NODE_ENV !== "production") {
    violations.push({
      type: "fatal",
      code: "NODE_ENV_MUST_BE_PRODUCTION",
      message:
        "[SECURITY] This deployment appears to be production (SHOPIFY_API_SECRET set and SHOPIFY_APP_URL is https and non-localhost), but NODE_ENV is not 'production'. Set NODE_ENV=production to proceed.",
    });
  }

  if (productionEnv && process.env.ALLOW_UNSIGNED_PIXEL_EVENTS === "true") {
    violations.push({
      type: "fatal",
  ...
  if (productionEnv) {
    if (!process.env.ENCRYPTION_SECRET) {
      violations.push({
        ...
        message:
          "[SECURITY] ENCRYPTION_SECRET is not set in production. " +
    ...
    if (!process.env.CRON_SECRET) {
      violations.push({
        ...
        message:
          "[SECURITY] CRON_SECRET is not set in production. " +
    ...
    if (!process.env.ENCRYPTION_SALT) {
      violations.push({
        ...
        message:
          "[SECURITY] ENCRYPTION_SALT is not set in production. " +
    ...
    if (appUrl && !appUrl.startsWith("https://")) {
      violations.push({
        ...
        message:
          "[SECURITY] SHOPIFY_APP_URL must use HTTPS in production. " +
    ...
    if (allowNullOrigin !== undefined && allowNullOrigin !== "") {
      ...
      if (!["true", "1", "false", "0"].includes(normalized)) {
        violations.push({
          ...
          message:
            "[P0-2 CONFIGURATION ERROR] PIXEL_ALLOW_NULL_ORIGIN_WITH_SIGNATURE_ONLY has invalid value. " +
    ...
    if (process.env.FEATURE_DEBUG_LOGGING === "true") {
      violations.push({
        ...
        message:
          "[SECURITY] FEATURE_DEBUG_LOGGING=true in production. " +
    ...
    if (process.env.DEFAULT_CONSENT_STRATEGY === "weak") {
      violations.push({
        ...
        message:
          "[COMPLIANCE] DEFAULT_CONSENT_STRATEGY=weak in production. " +
    ...
    if (process.env.TRUST_PROXY !== "true") {
      violations.push({
        ...
        message:
          "[P0-2 SECURITY] TRUST_PROXY must be 'true' in production. " +
    ...
    if (process.env.ALLOW_MEMORY_REDIS_IN_PROD === "true") {
      violations.push({
        type: "fatal",
        code: "ALLOW_MEMORY_REDIS_IN_PROD_FORBIDDEN_IN_PRODUCTION",
        message: "[SECURITY] ALLOW_MEMORY_REDIS_IN_PROD=true is set in production. Remove it to proceed.",
      });
    }
    if (process.env.LOCAL_DEV === "true" || process.env.LOCAL_DEV === "1") {
      violations.push({
        ...
        message:
          "[SECURITY] LOCAL_DEV=true or LOCAL_DEV=1 is set in production. " +
    ...
    if (value) {
      for (const pattern of suspiciousPatterns) {
        if (pattern.test(value)) {
          violations.push({
            type: productionEnv ? "fatal" : "warning",
            code: `SUSPICIOUS_SECRET_${secretName}`,
            message:
              `[SECURITY] ${secretName} appears to be a placeholder or default value. ` +
  ...
  try {
    ...
    for (const config of configsWithLegacy) {
      if (config.credentials_legacy && typeof config.credentials_legacy === "object") {
        violations.push({
          ...
          message:
            `[P0-3 SECURITY VIOLATION] Legacy plaintext credentials detected in production for PixelConfig ${config.id} (platform: ${config.platform}, shopId: ${config.shopId}). ` +
    ...
    if (error && typeof error === "object" && "code" in error && error.code === "P2022") {
      ...
      if (errorMessage.includes("credentials_legacy") && errorMessage.includes("does not exist")) {
        logger.warn(
          "PixelConfig.credentials_legacy column does not exist in database. Skipping legacy credentials check.",
          {
            error: errorMessage,
          }
        );
    ...
    logger.warn("Failed to check for legacy plaintext credentials during startup", {
