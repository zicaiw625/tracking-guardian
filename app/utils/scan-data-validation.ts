...
export function validateAdditionalScriptsPatterns(patterns: unknown): Array<{ platform: string; content: string }> {
  ...
  return patterns.filter((p: unknown): p is { platform: string; content: string } => {
    ...
    return typeof pattern.platform === "string" && typeof pattern.content === "string";
  });
}
