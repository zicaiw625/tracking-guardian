...
const blockedShops = new Map<string, {
    blockedAt: number;
    reason: string;
}>();
const BLOCKED_SHOP_COOLDOWN_MS = 10 * 60 * 1000;
export function trackAnomaly(shopDomain: string, type: "invalid_key" | "invalid_origin" | "invalid_timestamp"): {
    shouldBlock: boolean;
    reason?: string;
    ...
    const totalAnomalies = tracker.invalidKeyCount +
        tracker.invalidOriginCount +
        tracker.invalidTimestampCount;
...
export function getAnomalyStats(): Array<{
    ...
    return stats.sort((a, b) => (b.invalidKeyCount + b.invalidOriginCount + b.invalidTimestampCount) -
...
export function checkRateLimit(request: Request, endpoint: string, customConfig?: Partial<RateLimitConfig>): {
    isLimited: boolean;
    remaining: number;
    resetTime: number;
    retryAfter: number;
...
export async function checkRateLimitAsync(request: Request, endpoint: string, customConfig?: Partial<RateLimitConfig>): Promise<{
    isLimited: boolean;
    remaining: number;
    resetTime: number;
    retryAfter: number;
...
export function addRateLimitHeaders(response: Response, rateLimit: {
    remaining: number;
    resetTime: number;
    maxRequests?: number;
}): Response {
...
export function withRateLimit<T>(endpoint: string, handler: (args: {
    request: Request;
}) => Promise<T>, customConfig?: Partial<RateLimitConfig>): (args: {
    request: Request;
}) => Promise<T | Response> {
    return async (args) => {
