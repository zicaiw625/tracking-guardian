  ...
  if (shop.accessTokenEncrypted) {
    try {
      ...
      if (error instanceof TokenDecryptionError) {
        logger.warn(
          `[Shop Access] Failed to decrypt accessToken for shop ${shop.shopDomain}. ` + "Re-authentication required."
        );
...
export async function getShopForVerificationEncrypted(
  shopDomain: string
): Promise<ShopVerificationDataEncrypted | null> {
  const shop = await prisma.shop.findUnique({
    where: { shopDomain },
    select: {
      id: true,
...
export function decryptShopVerificationData(encrypted: ShopVerificationDataEncrypted): ShopVerificationData {
  const currentSecret = encrypted.ingestionSecret ? decryptIngestionSecret(encrypted.ingestionSecret) : null;
...
export async function getShopForVerification(shopDomain: string): Promise<ShopVerificationData | null> {
  ...
  const currentSecret = shop.ingestionSecret ? decryptIngestionSecret(shop.ingestionSecret) : null;
  let previousSecret: string | null = null;
...
export function decryptShopWithPixelConfigs(encrypted: ShopWithPixelConfigsEncrypted): ShopWithPixelConfigs {
  const currentSecret = encrypted.ingestionSecret ? decryptIngestionSecret(encrypted.ingestionSecret) : null;
  let previousSecret: string | null = null;
  if (
    encrypted.previousIngestionSecret &&
...
export function verifyWithGraceWindow(
  ...
  if (
    ...
    verifyFn(shop.previousIngestionSecret)
  ) {
    logger.info(
...
export async function verifyWithGraceWindowAsync(
  shop: ShopVerificationData,
  verifyFn: (secret: string) => Promise<boolean>
): Promise<{
  matched: boolean;
  usedPreviousSecret: boolean;
}> {
  ...
  if (
    ...
    shop.previousSecretExpiry &&
    new Date() < shop.previousSecretExpiry &&
    (await verifyFn(shop.previousIngestionSecret))
  ) {
    logger.info(
      `[Grace Window] Request verified using previous secret for ${shop.shopDomain}. ` +
        `Expires: ${shop.previousSecretExpiry?.toISOString()}`
