...
export const appMetrics = {
  ...
  pixelEventReceived: (shopDomain: string) => incrementCounter("pixel_event_received_total", { shop: shopDomain }),
  pixelEventRejected: (reason: string) => incrementCounter("pixel_event_rejected_total", { reason }),
  webhookProcessed: (topic: string, status: "success" | "failed") =>
    incrementCounter("webhook_processed_total", { topic, status }),
  webhookDuration: (topic: string, durationMs: number) => recordHistogram("webhook_duration_ms", durationMs, { topic }),
  capiEventSent: (platform: string, status: "success" | "failed") =>
    incrementCounter("capi_event_sent_total", { platform, status }),
  capiLatency: (platform: string, latencyMs: number) => recordHistogram("capi_latency_ms", latencyMs, { platform }),
  dbQueryDuration: (operation: string, durationMs: number) =>
    recordHistogram("db_query_duration_ms", durationMs, { operation }),
  rateLimitHit: (endpoint: string) => incrementCounter("rate_limit_hit_total", { endpoint }),
  activeJobs: (count: number) => setGauge("active_jobs", count),
  pendingRetries: (count: number) => setGauge("pending_retries", count),
  pxIngestAccepted: (shopDomain: string) => incrementCounter("px_ingest_accepted_count", { shop: shopDomain }),
  ...
  setInterval(() => {
    ...
    if (metricsCount > 0) {
      logger.info("[METRICS] Aggregated metrics", {
        ...
        sample: Object.entries(aggregated)
          .slice(0, 10)
          .map(([k, v]) => ({
            metric: k,
            count: v.count,
