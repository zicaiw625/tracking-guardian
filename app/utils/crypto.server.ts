...
function readScryptParams(): { N: number; r: number; p: number; maxmem: number } {
  ...
  const computedMaxmem =
    maxmemBytesEnv != null ? maxmemBytesEnv : maxmemMb != null ? maxmemMb * 1024 * 1024 : defaultMaxmem;
...
export function validateEncryptionConfig(): {
  ...
  if (
    isProduction &&
    secretSource === "ENCRYPTION_SECRET" &&
    warnings.some((w) => w.includes("ENCRYPTION_SECRET is shorter than recommended"))
  ) {
    throw new Error(
      "ENCRYPTION_SECRET must be at least 32 characters in production. Generate with: openssl rand -base64 32"
    );
  }
...
export const IV_LENGTH_V1 = 16;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const AUTH_TAG_LENGTH = 16;

export type EncryptionVersion = "v1" | "v2";

export function encrypt(plaintext: string, version: EncryptionVersion = "v2"): string {
  const key = getEncryptionKey();
  const ivLength = version === "v2" ? IV_LENGTH : IV_LENGTH_V1;
...
export function decrypt(encryptedData: string): string {
  ...
  if (ivLength !== IV_LENGTH && ivLength !== IV_LENGTH_V1) {
    throw new Error(`Invalid IV length: ${ivLength} bytes (expected ${IV_LENGTH_V1} for v1 or ${IV_LENGTH} for v2)`);
  }
...
export function generateEventId(orderId: string, eventType: string, shopDomain?: string): string {
  ...
  return createHash("sha256").update(hashInput, "utf8").digest("hex").substring(0, 32);
}
