import type { JobStatusType, TrustLevelType, PlatformType, ConsentStrategyType } from "./enums";
...
export function parseCapiInput(json: unknown): CapiInputJson | null {
  ...
  return {
    ...
    shopifyOrderId:
      typeof data.shopifyOrderId === "number" || typeof data.shopifyOrderId === "string"
        ? data.shopifyOrderId
        : undefined,
  };
}
...
export function parseTrustMetadata(json: unknown): TrustMetadataJson | null {
  ...
  return {
    ...
    webhookHasCheckoutToken:
      typeof data.webhookHasCheckoutToken === "boolean" ? data.webhookHasCheckoutToken : undefined,
...
export type AlertSettingsJson = EmailAlertSettingsJson | SlackAlertSettingsJson | TelegramAlertSettingsJson;
...
export function parsePixelClientConfig(json: unknown): PixelClientConfigJson | null {
  ...
  return {
    ...
    conversionLabels: Array.isArray(data.conversionLabels)
      ...
      : undefined,
    eventMappings:
      data.eventMappings && typeof data.eventMappings === "object"
  ...
  return {
    ...
    error:
      data.error && typeof data.error === "object"
        ? {
            code:
              typeof (data.error as Record<string, unknown>).code === "number" ||
...
export function createJsonArrayParser<T>(itemParser: (item: unknown) => T | null): (json: unknown) => T[] {
  return (json: unknown): T[] => {
    if (!Array.isArray(json)) {
      return [];
    }
    return json.map(itemParser).filter((item): item is T => item !== null);
  };
}
...
export function parseConsentStateResult(json: unknown): Result<ConsentStateJson, JsonParseError> {
  ...
  const saleOfData =
    data.saleOfDataAllowed !== undefined
      ? typeof data.saleOfDataAllowed === "boolean"
        ? data.saleOfDataAllowed
        : undefined
      : typeof data.saleOfData === "boolean"
        ? data.saleOfData
        : undefined;
  return ok({
    marketing: typeof data.marketing === "boolean" ? data.marketing : undefined,
